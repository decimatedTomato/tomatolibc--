  ...
  public:
    ...
    SharedPtr(const SharedPtr &rhs) : data(rhs.data), rc(rhs.rc)
    {
        ...
        cout << "Copied data " << data << " to " << rhs.data
             << " and reference count " << *rc << " from " << rc << " to "
             << rhs.rc << endl;
    ...
    {
        cout << "Exchanged data and reference count between this and rhs in "
                "initializer list"
             << endl;
    }
    SharedPtr &operator=(SharedPtr &&rhs) noexcept
    {
        if (this != &rhs)
        {
            cout << "Moving data " << *rhs.data << " from " << rhs.data
                 << " to " << data << endl;
    ...
    {
        if (!data)
            throw "Tried to dereference moved out Shared_ptr";
        return *data;
    }
